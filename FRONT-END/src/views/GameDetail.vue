...
export default {
  ...
  mounted () {
    ...
    console.log('OLHA MINHA PROP AQUI', this.game, this.allGames, this.allTeams)
    this.newGames = this.$store.getters.getGames
    // this.teams = this.$store.getters.getTeams
    // var newGames = this.allGames
    // for (var x in newGames) {
    //   if (
    //     (newGames[x].id_team1 === this.allTeams[game[0].id_team1-1].id &&
    //     newGames[x].id_team2 === this.allTeams[game[0].id_team2-1].id) || 
    //     (newGames[x].id_team2 === this.allTeams[game[0].id_team1-1].id &&
    //     newGames[x].id_team1 === this.allTeams[game[0].id_team2-1].id)
    //   ) {
    //     this.specificGames.push(newGames[x])
    //   }
    // }
  }
}
